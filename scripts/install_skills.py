#!/usr/bin/env python3
"""
PKB Starter -- Optional Skill Installer (v0.3.0)
...
> Installed by PKB Starter install_skills.py v0.3.0 on {today}
...
---
*Generated by PKB Starter install_skills.py v0.3.0*
"""
    if not dry_run:
...
def update_skill_links(target: Path, entries: list, results: list, dry_run: bool = False):
    ...
    lines = [
        ...
        "",
        f"> Auto-generated by PKB Starter install_skills.py v0.3.0 on {today}.",
    ...
    lines.append("---")
    lines.append("*Generated by PKB Starter v0.3.0*")
    lines.append("")
...
def print_install_plan(entries: list, to_install: list, skipped: list,
    ...
    print()
    print("=" * 70)
    print(f"  PKB Skill Installer v0.3.0 -- Profile: {profile}")
    print("=" * 70)
    print()
    ...
    if skipped:
        ...
        for entry in skipped:
            ...
            if risk == "REFERENCE_ONLY":
                ...
                reason = "high risk (use --enable-risky)"
            else:
...
def print_install_report(results: list, skipped_entries: list, dry_run: bool):
    ...
    for r in results:
        if r["status"] == "installed":
            ...
            print(f"  [OK]    {r['id']} -> {path}")
        elif r["status"] == "would_install":
            path = r.get("vendor_path", "")
            print(f"  [DRY]   {r['id']} -> {path}")
    ...
    total_skipped = len(skipped_results) + len(skipped_entries)
    print()
    print(f"  Summary: {len(succeeded)} installed, {len(failed)} failed, {total_skipped} skipped")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PKB Starter -- Optional Skill Installer (v0.3.0)",
