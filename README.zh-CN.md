...
>
> **当前版本**：v0.6.14-alpha | 🆕 **pkb_retrieve v3.0**：混合检索（BM25 + 向量 + Cross-encoder）| **kaoshibao_export**：题库转换器 | **全局查询**：`/ask-pkb` 任意窗口可用
...
**v0.6.14-alpha** — 当前版本。混合检索引擎（BM25 + 向量 RRF + Cross-encoder 三阶段）、考试宝题库导出（kaoshibao_export）、批量英文论文元数据（batch_english_papers）、CNKI 批量下载 + WebVPN、agent-skills-catalog v2（~132 skills）、.gitignore 安全规则扩展。
