# PKB Starter ![version](https://img.shields.io/badge/version-v0.6.15--starter-blue)

> **一个命令管理你的知识。** `/pkb <任何东西>` — 丢入 URL、文件或想法，LLM 自动整理一切。
>
> **当前版本**：v0.6.15-starter | 🆕 **防退化系统 v1.0**：15 项 bug 修复 + E2E 测试套件 | **全局桥接**：`/ask-pkb` 任意窗口可用
...
**v0.6.15-starter** — 当前版本。防退化系统（15 项 hook 修复 + E2E 测试）、全局桥接引擎 v1.0、同步管线二进制处理 + 脱敏修复、Obsidian vault 迁移、manifest 完整性（120 mappings / 0 dup）。

**v0.6.14-starter** — 混合检索引擎（BM25 + 向量 RRF + Cross-encoder 三阶段）、批量英文论文元数据（batch_english_papers）、CNKI 批量下载 + WebVPN、agent-skills-catalog v2（~132 skills）、.gitignore 安全规则扩展。
