...
Query your PKB wiki from ANY Claude Code project:

1. Set the `PKB_ROOT` environment variable (same convention as `pkb.ps1`)
2. The skill is at `template/.claude/skills/ask-pkb/SKILL.md` — copy or symlink to `~/.claude/skills/ask-pkb/`
3. From any project: `/ask-pkb "your question"`

Path detection order: `PKB_ROOT` env var → auto-detect (walk up from cwd) → `~/.pkb/config.json`
