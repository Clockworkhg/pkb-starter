...
3. 对于长路径问题，使用更短的根路径（例如 `D:\PKB\` 代替 `D:\我的非常长的路径名称\PKB\`）
...
```
python scripts/install_skills.py --target "D:\我的知识库" --profile full --enable-risky
```
...
**解决方法**：
1. 运行：`python scripts/skill_manager.py --target "D:\我的知识库" --install z-skills`
2. 阅读显示的风险说明
...
**解决方法**：
1. 运行：`python scripts/skill_manager.py --target "D:\我的知识库" --audit`
...
2. 重新安装 PKB 模板：`python scripts/install.py "D:\我的知识库" --force`
...
3. 如果缺失，重新安装：`python scripts/install.py "D:\我的知识库" --force`
...
**解决方法**：
1. 先手动更新：`python scripts/update_pkb.py "D:\我的知识库"`
