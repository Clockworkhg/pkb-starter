...
- ❌ **不要在 v0.6.2-alpha 上运行 `/docs-update`**——等到更新到 v0.6.4-alpha 后再运行。

## 4. 如果你的更新源仍然是占位符

v0.6.2-alpha 安装时在 `pkb.config.json` 中写入了占位符 `starter_repo_url`：

```json
"starter_repo_url": "https://github.com/<your-username>/pkb-starter.git"
```

这会导致更新客户端失败并提示 "No valid starter_repo_url configured."
...
`--apply` 后，`starter_repo_url` 会更新到 `pkb.config.json`，之后可直接使用 `/update`。

如果你使用个人 fork，将 URL 替换为你的 fork 地址。

## 5. 安全更新到 v0.6.4-alpha

v0.6.4-alpha 修复了所有这些问题。使用内置更新客户端：
...
## 6. 如果在 v0.6.2-alpha 上已运行 /docs-update
...
2. **检查更新报告：**
   - 如果 `update_client_report.md` 列出核心文档冲突，接受 v0.6.4-alpha 模板版本（除非你故意自定义了它们）。
...
*本指南适用于 v0.6.2-alpha/v0.6.3-alpha → v0.6.4-alpha 迁移。如需一般更新信息，请参见 [UPDATING.md](../UPDATING.md)。*
