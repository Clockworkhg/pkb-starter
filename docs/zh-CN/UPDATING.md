...
- **v0.6.3-alpha**：当前。新装自洽、docs-update 安全修复、v0.6.2-alpha 恢复。
...
快速恢复：
```bash
python tools/pkb_update_client.py --checkout v0.6.3-alpha          # dry-run（安全）
python tools/pkb_update_client.py --checkout v0.6.3-alpha --apply  # 应用更改
```
...
在 `pkb.config.json` 中设置 `starter_repo_url` 为你的 pkb-starter fork，以支持在线更新：
...
如果未设置，使用 `--repo-url` 或 `--starter-path` 参数。
